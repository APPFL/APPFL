...
class MPIResultAnalyzer:
    ...
    def extract_metrics_from_profile(self, profile_path: str, rank: str, profile_type: str) -> Optional[ProfileMetrics]:
    ...
    def _parse_text_stats(self, profile_path: str, rank: str, profile_type: str) -> Optional[ProfileMetrics]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return ProfileMetrics(
                    ...
                    avg_allocation_size=total_allocated * 1024 * 1024 / max(total_allocations, 1),
    ...
    def collect_all_metrics(self) -> List[ProfileMetrics]:
        ...
        for rank, rank_profiles in self.profiles.items():
            for profile_type, profile_path in rank_profiles.items():
                ...
                metrics = self.extract_metrics_from_profile(profile_path, rank, profile_type)
                if metrics:
    ...
    def generate_comparison_plots(self):
        ...
        for metric in self.metrics:
            df_data.append({
                'rank': metric.rank,
                'rank_type': 'Server' if metric.rank == '0' else f'Client {metric.rank}',
    ...
    def _plot_memory_usage_comparison(self, df: pd.DataFrame):
        ...
        # Peak Memory Usage
        sns.barplot(data=df, x='rank_type', y='peak_memory_mb', hue='profile_type', ax=axes[0, 0])
        axes[0, 0].set_title('Peak Memory Usage by Rank')
        axes[0, 0].set_ylabel('Peak Memory (MB)')
        axes[0, 0].tick_params(axis='x', rotation=45)
        
        # Total Allocated Memory
        sns.barplot(data=df, x='rank_type', y='total_allocated_mb', hue='profile_type', ax=axes[0, 1])
        axes[0, 1].set_title('Total Allocated Memory by Rank')
        axes[0, 1].set_ylabel('Total Allocated (MB)')
        axes[0, 1].tick_params(axis='x', rotation=45)
        
        # Total Allocations
        sns.barplot(data=df, x='rank_type', y='total_allocations', hue='profile_type', ax=axes[1, 0])
        axes[1, 0].set_title('Total Number of Allocations')
        axes[1, 0].set_ylabel('Total Allocations')
        axes[1, 0].tick_params(axis='x', rotation=45)
        
        # Allocation Rate
        sns.barplot(data=df, x='rank_type', y='allocation_rate', hue='profile_type', ax=axes[1, 1])
    ...
    def _plot_allocation_patterns(self, df: pd.DataFrame):
        ...
        sns.scatterplot(data=df, x='duration', y='peak_memory_mb', 
                       hue='profile_type', style='rank_type', s=100, ax=axes[1])
        axes[1].set_title('Profile Duration vs Peak Memory')
        axes[1].set_xlabel('Duration (seconds)')
        axes[1].set_ylabel('Peak Memory (MB)')
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'allocation_patterns.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def _plot_optimization_impact(self, df: pd.DataFrame):
        ...
        for rank_type in df['rank_type'].unique():
            ...
            if len(original) > 0 and len(optimized) > 0:
                ...
                for metric in ['peak_memory_mb', 'total_allocated_mb', 'total_allocations']:
                    if orig_row[metric] > 0:
                        improvement = ((orig_row[metric] - opt_row[metric]) / orig_row[metric]) * 100
        ...
        if improvement_data:
            ...
            sns.barplot(data=improvement_df, x='rank_type', y='improvement_percent', hue='metric')
            ...
            plt.tight_layout()
            plt.savefig(self.output_dir / 'optimization_impact.png', dpi=300, bbox_inches='tight')
            plt.show()
    ...
    def _plot_rank_comparison(self, df: pd.DataFrame):
        ...
        metrics = ['peak_memory_mb', 'total_allocated_mb', 'total_allocations', 'allocation_rate']
        titles = ['Peak Memory Usage', 'Total Allocated Memory', 'Total Allocations', 'Allocation Rate']
        y_labels = ['Memory (MB)', 'Memory (MB)', 'Number of Allocations', 'Allocations/sec']
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'rank_comparison.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def generate_summary_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            for rank in sorted(rank_metrics.keys()):
                ...
                for profile_type in ['original', 'optimized']:
                    if profile_type in rank_data:
                        ...
                        f.write(f"  Average Allocation Size: {metric.avg_allocation_size:.2f} bytes\n")
                        f.write(f"  Allocation Rate: {metric.allocation_rate:.2f} allocs/sec\n")
                        f.write(f"  Profile Duration: {metric.profile_duration:.2f} seconds\n")
            ...
            total_orig_peak = sum(m.peak_memory for m in self.metrics if m.profile_type == 'original')
            total_opt_peak = sum(m.peak_memory for m in self.metrics if m.profile_type == 'optimized')
            
            total_orig_allocated = sum(m.total_allocated for m in self.metrics if m.profile_type == 'original')
            total_opt_allocated = sum(m.total_allocated for m in self.metrics if m.profile_type == 'optimized')
            
            if total_orig_peak > 0 and total_opt_peak > 0:
                peak_improvement = ((total_orig_peak - total_opt_peak) / total_orig_peak) * 100
                f.write(f"Overall Peak Memory Improvement: {peak_improvement:.1f}%\n")
            
            if total_orig_allocated > 0 and total_opt_allocated > 0:
                allocated_improvement = ((total_orig_allocated - total_opt_allocated) / total_orig_allocated) * 100
                f.write(f"Overall Allocated Memory Improvement: {allocated_improvement:.1f}%\n")
            
            f.write(f"\nKey Optimization Features:\n")
            f.write(f"  • Memory-efficient model serialization using context managers\n")
            f.write(f"  • Strategic garbage collection during MPI operations\n")
            f.write(f"  • CPU-first model loading to reduce memory pressure\n")
            f.write(f"  • Immediate cleanup of communication buffers\n")
            f.write(f"  • Resource cleanup after MPI message processing\n")
            
            f.write(f"\nGenerated Files:\n")
            f.write(f"  • memory_usage_comparison.png - Memory usage comparison charts\n")
            f.write(f"  • allocation_patterns.png - Memory allocation pattern analysis\n")
            f.write(f"  • optimization_impact.png - Optimization impact visualization\n")
            f.write(f"  • rank_comparison.png - Performance comparison between ranks\n")
            f.write(f"  • memory_optimization_report.txt - This comprehensive report\n")
