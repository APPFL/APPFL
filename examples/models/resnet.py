...
def resnet18(num_classes=None, num_channel=3):
    if num_channel == 1:
        if num_classes is not None:
            return ResNet(block=BasicBlock, layers=[2, 2, 2, 2],num_classes=num_classes, grayscale=True)
        else:
            return ResNet(block=BasicBlock, layers=[2, 2, 2, 2], grayscale=True)    
    else:
        if num_classes is not None:
