...
class QueueAsyncFL(BaseFLAlgorithm):
    def run(self):
        ...
        delay_mode = str(
            getattr(getattr(cfg, "algo", {}), "delay_mode", "simulate")
        ).lower()  # simulate|sleep

        # Tsync
        if str(cfg.case2.Tsync).lower() == "auto":
            step_times = [1.0 / max(1e-6, ck[cid]) for cid in client_ids]
            t_q = float(
                __import__("numpy").quantile(step_times, float(cfg.case2.quantile))
            )
            Tsync = (
                float(cfg.case2.theta)
                + float(cfg.case2.q_init)
                + float(cfg.case2.Etarget) * t_q
            )
        ...
        lr_clip_lo = float(cfg.optim.lr_clip_lo)
        lr_clip_hi = float(cfg.optim.lr_clip_hi)

        broadcast_when = str(
            getattr(getattr(cfg, "algo", {}), "broadcast_when", "next_round")
        ).lower()
        validate_cid = str(
            getattr(getattr(cfg, "algo", {}), "validate_on_client_id", "Client1")
        )
        ...
        for r in range(1, int(cfg.system.num_rounds) + 1):
            ...
            for cid in client_ids:
                ...
                eta_map[cid] = max(
                    eta_base * lr_clip_lo, min(eta, eta_base * lr_clip_hi)
                )
            ...
            for res in results:
                ...
                item = {
                    ...
                    "Ek": Ek_map[cid],
                    "eta": eta_map[cid],
                    "arrival_offset": arrivals[cid] - B_r,
                }
                if admitted:
            ...
            if len(agg_items) > 0:
                ...
                for x in agg_items:
                    accum = state_add(
                        accum, state_scale(x["delta"], x["w"] / W), alpha=1.0
                    )
            ...
            # logs
            late_str = (
                ", ".join([f"{x['cid']}[from={x['origin_round']}]" for x in carry_over])
                if carry_over
                else "[]"
            )
            log_line(
                log_path,
                f"[Round {r}] Tsync={Tsync:.2f}, alpha={alpha}, theta={theta}, carried_in={len(carry_over)}; late_out={len(late_this_round)}",
            )
            log_line(
                log_path, f"  A_on_time(r={r}) = {[it['cid'] for it in admitted_now]}"
            )
            log_line(log_path, f"  A_late(r={r})   = {late_str}")
            for cid in client_ids:
                log_line(
                    log_path,
                    f"  {cid}: q_hat={q_hat[cid]:.3f}, Jk={Jk_map[cid]:.3f}, Ek={Ek_map[cid]}, ck={ck[cid]:.2f}, eta={eta_map[cid]:.5f}",
                )
            log_line(
                log_path, f"  val[{validate_cid}]: loss={vloss:.4f} acc={vacc:.4f}"
            )
