...
def set_server_params(server, new_state: dict):
    ...
    if hasattr(server, "set_parameters"):
        try: server.set_parameters(new_state); return
        except TypeError: server.set_parameters((new_state,)); return
        except Exception: pass
    if hasattr(server, "load_parameters"):
        try: server.load_parameters(new_state); return
        except Exception: pass
    for attr in ("model", "_model", "global_model"):
        ...
        if m is not None and hasattr(m, "load_state_dict"):
            ...
            m.load_state_dict(cpu_state, strict=True); return
    if hasattr(server, "_set_parameters"):
        try: server._set_parameters(new_state); return
        except Exception: pass
