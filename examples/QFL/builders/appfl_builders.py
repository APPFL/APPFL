...
def ensure_client_ready(c: ClientAgent):
    if getattr(c, "trainer", None) is not None: return
    if hasattr(c, "_prepare") and callable(c._prepare): c._prepare()
    elif hasattr(c, "prepare") and callable(c.prepare): c.prepare()
    if getattr(c, "trainer", None) is None:
        raise RuntimeError("Client trainer not initialized. Check client YAML sections (trainer/model/loss/metric/data).")

def fix_metric_symbol(path: str, name: str) -> str:
    spec = importlib.util.spec_from_file_location("metric_mod", path)
    if spec is None or spec.loader is None: return name
    mod = importlib.util.module_from_spec(spec)
    try: spec.loader.exec_module(mod)
    except Exception: return name
    for cand in (name, "ACC", "acc", "accuracy", "Accuracy"):
        if hasattr(mod, cand): return cand
    return name
...
def build_server(cfg: DictConfig) -> ServerAgent:
    ...
    if _CustomServerAgent is not None:
        return _CustomServerAgent(s_cfg, threshold=1, momentum_coeff=float(cfg.aggregation.momentum_coeff))
...
def build_clients(cfg: DictConfig):
    ...
    model_path  = str(getattr(getattr(res, "model", {}), "path", "./resources/model/simple_cnn.py"))
    model_name  = str(getattr(getattr(res, "model", {}), "name", "SimpleCNN"))
    loss_path   = str(getattr(getattr(res, "loss", {}),  "path", "./resources/loss/celoss.py"))
    loss_name   = str(getattr(getattr(res, "loss", {}),  "name", "CELoss"))
    metric_path = str(getattr(getattr(res, "metric", {}),"path", "./resources/metric/acc.py"))
    metric_name = str(getattr(getattr(res, "metric", {}),"name", "ACC"))
    data_path   = str(getattr(getattr(res, "dataset", {}),"path", "./resources/dataset/custom_mnist_dataset.py"))
    data_name   = str(getattr(getattr(res, "dataset", {}),"name", "get_custom_mnist"))

    for p, lbl in [(model_path,"Model file"), (loss_path,"Loss file"), (metric_path,"Metric file"), (data_path,"Dataset file")]:
        require_path(lbl, p)
    ...
    for i in range(n):
        ...
        dkw.num_clients = n
        dkw.client_id = i
        dkw.partition_strategy = str(getattr(getattr(cfg, "data", {}), "partition", "iid"))
