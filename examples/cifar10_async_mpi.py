...
import torch
import argparse
import appfl.run_mpi_cpas as rmc
...
def main():
    ...
    cfg.save_model_state_dict = False
    cfg.output_dirname = "./outputs_%s_%s_%s_%s_%s" % (
        args.dataset, 
        args.partition,
        args.model, 
        args.server, 
    ...
    if comm_rank == 0:
        if args.server.startswith("ServerFedCPAS"):
            rmc.run_server(cfg, comm, model, loss_fn, args.num_clients, test_dataset, args.dataset)
        else:
