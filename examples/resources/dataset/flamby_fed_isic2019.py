...
try:
    import boto3
    from botocore.exceptions import ClientError # Import specific error for better handling
except ImportError:
    print("WARNING: boto3 not found. S3 functionality will be unavailable. "
...
class Isic2019Raw(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if use_s3:
            # --- S3 Initialization ---
            if boto3 is None:
                raise ImportError("boto3 is required for S3 support. Please install it: pip install boto3")
            ...
            try:
                s3_response = self.s3_client.get_object(Bucket=self.s3_bucket, Key=self.s3_metadata_key)
                ...
                if error_code == 'NoSuchKey':
                     raise FileNotFoundError(f"Metadata file not found in S3 at key: {self.s3_metadata_key} in bucket {self.s3_bucket}")
                elif error_code == 'AccessDenied':
                    raise PermissionError(f"Access denied when trying to read S3 key: {self.s3_metadata_key}")
                else:
                    raise IOError(f"S3 Error reading metadata from s3://{s3_bucket}/{s3_metadata_key}: {e}")
            except Exception as e:
                 raise IOError(f"Error parsing metadata CSV from S3: {e}")

        else:
            # --- Local Disk Initialization ---
            print("Using local file system.")
            if not data_path or not os.path.isdir(data_path):
                 raise ValueError(f"A valid local 'data_path' must be provided when use_s3 is False. Got: {data_path}")
            ...
            self.local_image_dir = os.path.join(data_path, "ISIC_2019_Training_Input_preprocessed")
            ...
            if not os.path.exists(self.local_metadata_path):
                 raise FileNotFoundError(f"Local metadata file not found at: {self.local_metadata_path}")
            if not os.path.isdir(self.local_image_dir):
                 raise NotADirectoryError(f"Local image directory not found at: {self.local_image_dir}")
            ...
            try:
                ...
                print(f"Metadata loaded successfully from {self.local_metadata_path}.")
                 ...
                 raise IOError(f"Error reading local metadata file {self.local_metadata_path}: {e}")
        ...
        self.targets = self.metadata_df['target'].tolist() # Use .tolist() for consistency
    ...
    def __getitem__(self, idx):
        ...
        if idx >= len(self.image_identifiers):
             raise IndexError(f"Index {idx} out of bounds for dataset length {len(self.image_identifiers)}")
        ...
        try:
            if self.use_s3:
                ...
                try:
                    s3_response = self.s3_client.get_object(Bucket=self.s3_bucket, Key=s3_key)
                    ...
                    if error_code == 'NoSuchKey':
                        ...
                        raise FileNotFoundError(
                         ...
                         raise IOError(f"S3 Error loading image {s3_key}: {e}")
            ...
            try:
                 if self.augmentations:
                      ...
                      elif any(hasattr(t, 'height') for t in getattr(self.augmentations, 'transforms', [])):
                           sz = next(t.height for t in self.augmentations.transforms if hasattr(t, 'height'))
            except:
            ...
            print(f"Error processing item idx={idx}, image_name='{image_name}': {type(e).__name__} - {e}")
            sz = 200
            try: # Reuse size detection logic
                 if self.augmentations:
                      if hasattr(self.augmentations, 'height'): sz = self.augmentations.height
                      elif any(hasattr(t, 'height') for t in getattr(self.augmentations, 'transforms', [])):
                           sz = next(t.height for t in self.augmentations.transforms if hasattr(t, 'height'))
            except: pass
            dummy_image = torch.zeros((3, sz, sz), dtype=self.X_dtype)
    ...
    def __init__(
        ...
        if train:
            ...
            augmentations = albumentations.Compose([
                albumentations.RandomScale(scale_limit=0.07, p=0.5), # Added p for probability
                albumentations.Rotate(limit=50, p=0.5), # Added p
                albumentations.RandomBrightnessContrast(brightness_limit=0.15, contrast_limit=0.1, p=0.5), # Added p
                ...
                albumentations.CoarseDropout(
                    max_holes=random.randint(1, 8), # Keep random hole count per image
                    max_height=16, max_width=16,
                    min_holes=1, min_height=8, min_width=8, # Added min values for example
        ...
        if self.metadata_df is None:
             raise RuntimeError("Base class (Isic2019Raw) did not load metadata correctly.")
        ...
        if not all(col in self.metadata_df.columns for col in required_filter_cols):
             raise ValueError(f"Metadata CSV must contain columns: {required_filter_cols} for filtering. "
                              f"Found: {self.metadata_df.columns.tolist()}")
        ...
        if self.pooled:
            ...
            df_filtered = df_to_filter.query("fold == @self.train_test").reset_index(drop=True)
            print(f"Filtering for pooled '{self.train_test}' split using 'fold' column.")
        else:
            # Filter by specific center and train/test split using 'fold2' column
            if not (0 <= center < 6): # Assuming 6 centers (0-5)
                 raise ValueError(f"Invalid center specified: {center}. Must be between 0 and 5.")
            ...
            df_filtered = df_to_filter.query("fold2 == @split_key").reset_index(drop=True)
            print(f"Filtering for center {center}, '{self.train_test}' split using 'fold2' column (key='{split_key}').")
        ...
        if len(self.image_identifiers) == 0:
             print("WARNING: Dataset is empty after filtering. Check metadata and filter criteria.")
