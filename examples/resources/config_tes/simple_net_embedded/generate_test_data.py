...
def generate_client_dataset(client_id: int, num_samples: int = 100, num_features: int = 8, num_classes: int = 2):
...
def save_dataset(X, y, output_dir: str, client_id: int):
    ...
    pd.DataFrame(X).to_csv(features_file, index=False, header=[f"feature_{i}" for i in range(X.shape[1])])
...
def main():
    ...
    print(f"Set volume_mounts in client configs to mount {base_data_dir}/client_X to /data")
