...
test_dataset = Dataset(
        torch.FloatTensor(test_data_raw["x"]), torch.FloatTensor(test_data_raw["y"]).reshape(-1,1)
    )
...
""" get model """
file = "./save_models/Covid_Binary_Isabelle_FedAvg_LR_0.005_Round_5.pt"
...
with torch.no_grad():
    for img, target in dataloader:        
        ...
        plt.figure() 
        lw = 2
        colors = ["darkgreen", "navy", "deeppink", "aqua", "darkorange", "cornflowerblue"]        
        plt.plot(
            fpr,
            tpr,
            color="darkorange",
