...
def plot_mean_var(datasets, color):
    ...
    plt.plot(time_points, mean_accuracies, label='Mean accuracy', color=color)
    plt.fill_between(time_points, mean_accuracies - std_accuracies, mean_accuracies + std_accuracies, color=color, alpha=0.2, label='Standard deviation')
    plt.xlabel('Time')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.title('Mean and Standard Deviation of Accuracy over Time')

def plot_mean_var_among_datasets(datasets_list, colors, save_path=None):
    """Plot mean and var for different sets of data"""
    plt.figure(figsize=(10,6))
    for i in range(len(datasets_list)):
        plot_mean_var(datasets_list[i], colors[i])
    if save_path:
        plt.savefig(save_path)
    else:
        plt.show()

def plot_dirs(dirs, colors, save_path=None):
    ...
    plot_mean_var_among_datasets(data, colors, save_path)
...
if __name__ == "__main__":
    ...
    plot_dirs(args.raw_result_dirs, colors, save_path)