...
class DummyCIFAR10Dataset(Dataset):
    ...
    def __init__(self, num_samples=64, num_classes=10, image_size=(3, 32, 32), seed=42):
        ...
        print(f"Created dummy CIFAR-10 dataset: {num_samples} samples, shape {image_size}")
...
def get_dummy_cifar10(
    ...
    **kwargs
    ...
    print(f"Client {client_id} dummy datasets: train={len(train_dataset)}, test={len(test_dataset)}")
    print(f"Memory footprint: ~{samples_per_client * 3 * 32 * 32 * 4 / 1024 / 1024:.1f} MB per client")
