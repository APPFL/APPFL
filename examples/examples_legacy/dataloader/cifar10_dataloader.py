    ...
    **kwargs,
    ...
    if comm_rank == 0:
        test_data_raw = torchvision.datasets.CIFAR10(
            dir, download=True, train=False, transform=test_transform("CIFAR10")
        )
    if comm is not None:
        comm.Barrier()
    if comm_rank > 0:
        test_data_raw = torchvision.datasets.CIFAR10(
            dir, download=False, train=False, transform=test_transform("CIFAR10")
        )
    ...
    train_data_raw = torchvision.datasets.CIFAR10(
        dir, download=False, train=True, transform=train_transform("CIFAR10")
    )
    ...
    if partition == "iid":
        train_datasets = iid_partition(
            train_data_raw,
            num_clients,
            visualization=visualization and comm_rank == 0,
            output=filename,
        )
    elif partition == "class_noiid":
        train_datasets = class_noiid_partition(
            train_data_raw,
            num_clients,
            visualization=visualization and comm_rank == 0,
            output=filename,
        ...
        train_datasets = dirichlet_noiid_partition(
            train_data_raw,
            num_clients,
            visualization=visualization and comm_rank == 0,
            output=filename,
