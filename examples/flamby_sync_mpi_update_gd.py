...
import torch
import argparse
import appfl.run_mpi_new as rmn
...
## server
parser.add_argument("--server", type=str, default="ServerFedAvgNew", 
                    choices=["ServerFedAvgNew"])
...
def main():
    ...
    print(cfg.fed.args.gradient_based)
    print(args.gradient_based)
    ...
    """ Running """
    if comm_rank == 0:
        rmn.run_server(cfg, comm, model, loss_fn, args.num_clients, test_dataset, args.dataset, metric)
    else:
        assert comm_size == args.num_clients + 1
        rmn.run_client(cfg, comm, model, loss_fn, args.num_clients, train_datasets, test_dataset, metric)
