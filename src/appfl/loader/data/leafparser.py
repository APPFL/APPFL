...
def _is_leaf_ready(dataset_root: Path) -> bool:
    ...
    ) or _has_json_files(dataset_root / "all_data")


def _prepare_leaf_data(config, dataset_key: str) -> Path:
    ...
    if not _as_bool(getattr(config, "download", True), default=True):
    ...
    if not _has_json_files(dataset_root / "all_data"):
        ...
        try:
            preprocess_mod = importlib.import_module(
                f"appfl.loader.data.leaf.preprocess.{dataset_key}"
            )
    ...
    if not (
        ...
        postprocess_leaf(
            ...
            raw_data_fraction=float(getattr(config, "pre_raw_data_fraction", 1.0)),
            min_samples_per_clients=int(
                getattr(config, "pre_min_samples_per_client", 2)
            ),
...
def _resolve_leaf_train_test(config, dataset_root: Path, dataset_key: str):
    ...
    return _split_from_all_data(
        ...
        seed=int(getattr(config, "data_seed", 42)),
        raw_data_fraction=float(getattr(config, "pre_raw_data_fraction", 1.0)),
        min_samples_per_client=int(getattr(config, "pre_min_samples_per_client", 2)),
    )


def _resolve_image_root(dataset_root: Path, dataset_key: str) -> Path | None:
...
def fetch_leaf(config):
    ...
    train_obj, test_obj = _resolve_leaf_train_test(config, dataset_root, dataset_key)
    ...
    image_root = _resolve_image_root(dataset_root, dataset_key)

    defaults = _DEFAULT_LEAF_META.get(dataset_key, {})
    seq_len = int(getattr(config, "seq_len", defaults.get("seq_len", 32)))
    num_embeddings = int(
        getattr(config, "num_embeddings", defaults.get("num_embeddings", 10000))
    )
    ...
    if first_train is not None:
        x0, _ = first_train[0]
        config.input_shape = tuple(getattr(x0, "shape", (1,)))
    else:
    ...
    return client_datasets, None, config
