...
def fetch_torchaudio_dataset(config):
    active_logger = resolve_dataset_logger(config, logger)
    tag = make_load_tag(str(config.dataset_name), benchmark="TORCHAUDIO")
    active_logger.info("[%s] resolving dataset class.", tag)
    ds_cls, ta = _resolve_torchaudio_dataset(config.dataset_name)
    if ta is None:
        raise RuntimeError("torchaudio is not installed.")
    if ds_cls is None:
        raise ValueError(f"Unknown torchaudio dataset: {config.dataset_name}")
    ...
    max_frames = int(getattr(config, "audio_num_frames", 16000))
