...
def preprocess(root):
    ...
    def _get_glove_vocab(path):
        ...
        lines = []
        with open(os.path.join(path, "raw", "glove.6B.300d.txt")) as file:
        ...
        embs = [[float(n) for n in line[1:]] for line in lines]
        embs.insert(0, [0.0 for _ in range(300)])  # for unknown token
    ...
    def _convert_to_json(path, raw_all, indices):
        def _cleanse(df):
            ...
            df = df.apply(
                lambda x: re.sub(
                    r"[0-9]+(?:st| st|n[d]| n[d]|rd| rd|th| th)",
                    " ",
                    str(x).strip(),
                )
            )
        ...
        user_id_map = {
            str_id: int_id
            for int_id, str_id in enumerate(raw_all.index.unique().tolist())
        }
        ...
        raw_all["text"] = (
            raw_all["text"].map(lambda x: _line_to_indices(x, indices)).astype("object")
        )
        ...
        raw_all = (
            ...
            .agg(
                {
                    "text": lambda x: [i for i in x],
                    "target": lambda y: [label for label in y],
                }
            )
    ...
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] Process GloVe embeddings (300 dim.)...!"
    )
    glove_vocab_indices = _get_glove_vocab(path)
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] ...finished processing GloVe embeddings!"
    )
