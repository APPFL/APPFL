...
def preprocess(root):
    ...
    def _parse_files(path):
        def _parse_class_files(path):
            ...
            _save_obj(
                class_files, os.path.join(path, "intermediate", "class_file_dirs")
            )
        ...
        def _parse_write_files(path):
            ...
            _save_obj(
                write_files, os.path.join(path, "intermediate", "write_file_dirs")
            )
    ...
    def _convert_to_json(path):
        ...
        # assign data
        for w, writer_images in writers:
            users.append(w)
            num_samples.append(len(writer_images))
            user_data[w] = {"x": [], "y": []}

            for f, c in writer_images:
    ...
    # parse files
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Extract files of raw images...!")
    _parse_files(path)
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] ...finished extracting files of raw images!"
    )

    # get file hashes
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Calculate image hashes...!")
    _get_hashes(path)
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] ...finished calculating image hashes!")

    # match by hashes
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] Assign class labels to write images...!"
    )
    _match_by_hashes(path)
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] ...finished assigning class labels to write images!"
    )

    # group images by writer
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Group images by write...!")
    _group_by_write(path)
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] ...finished grouping images by writer!")

    # convert to json format
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Convert data to json format...!")
