...
from appfl.comm.grpc import deserialize_model, create_grpc_channel
...
class GRPCDFLNodeConnectCommunicator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        grpc.channel_ready_future(channel).result(
            timeout=3600
        )  # use a long timeout here for DFL
    ...
    def invoke_custom_action(self, action: str, **kwargs) -> Dict:
        ...
        # vaseline555: also tried replacing above into
        # https://github.com/APPFL/APPFL/blob/9bae5a4090eb70014cb20c72883dfb930fc8d829/src/appfl/comm/grpc/grpc_client_communicator.py#L331
        # but it raised the same error as below...
        """
        grpc._channel._MultiThreadedRendezvous: <_MultiThreadedRendezvous of RPC that terminated with:
        status = StatusCode.UNKNOWN
        details = "Exception calling application: '_RequestIterator' object has no attribute 'action'"
        debug_error_string = "UNKNOWN:Error received from peer  {grpc_status:2, grpc_message:"Exception calling application: \'_RequestIterator\' object has no attribute \'action\'"}"
        """
        # if response.header.status == ServerStatus.ERROR:
        if response.code() == ServerStatus.ERROR:  # vaseline555: just a workaround..
            raise Exception("Neighbor node returned an error, stopping the node.")
        # if len(response.results) == 0: # vaseline555: just a workaround..
        if len(response.initial_metadata()) == 0:
            return {}
        else:
            try:
