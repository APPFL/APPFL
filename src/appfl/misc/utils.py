...
def add_params(named_param_1: OrderedDict, named_param_2: OrderedDict, layer_names: list = []) -> OrderedDict:
    result = copy.deepcopy(named_param_1)
    for name in layer_names:
        result[name] = named_param_2[name] - named_param_1[name]
    return result

def sub_params(named_param_1: OrderedDict, named_param_2: OrderedDict, layer_names: list = []) -> OrderedDict:
    result = copy.deepcopy(named_param_1)
    for name in layer_names:
        result[name] = named_param_1[name] - named_param_2[name]
    return result

def mul_params(named_param_1: OrderedDict, scalar: float, layer_names: list = []) -> OrderedDict:
    result = copy.deepcopy(named_param_1)
    for name in layer_names:
        result[name].mul_(torch.Tensor(scalar).to(result[name].device))
    return result

def norm_difference(named_param_1: OrderedDict, named_param_2: OrderedDict, norm_type: float = 1) -> torch.Tensor:
    named_param_1_flattened = torch.cat([pdata.detach().cpu().flatten() for _,pdata in named_param_1.items()])
    named_param_2_flattened = torch.cat([pdata.detach().cpu().flatten() for _,pdata in named_param_2.items()])
    return torch.linalg.norm(named_param_1_flattened-named_param_2_flattened,ord=norm_type)