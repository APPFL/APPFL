...
def efficient_tensor_aggregation(
    ...
    with torch.no_grad():
        ...
        # Aggregate tensors
        total_weight = len(tensors) if weights is None else sum(weights.values())
        
        for client_id, tensor_dict in tensors.items():
            weight = 1.0 / len(tensors) if weights is None else weights.get(client_id, 0.0)
