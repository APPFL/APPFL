...
def normalize_dataset(train_dataset, feature_range=(0, 1)):
    ...
    # Determine whether the input is a simple 2D feature matrix or multi-dimensional
    if len(data_input_np.shape) == 2:
        ...
        # Reshape to (N, -1) where N is the number of samples
        data_input_flattened = data_input_np.reshape(data_input_np.shape[0], -1)
        
        scalers = [MinMaxScaler(feature_range=feature_range) for _ in range(data_input_flattened.shape[1])]
        
        data_input_normalized_flattened = data_input_flattened.copy()
        for i in range(data_input_flattened.shape[1]):
            data_input_normalized_flattened[:, i] = scalers[i].fit_transform(data_input_flattened[:, i].reshape(-1, 1)).flatten()
        
        # Reshape back to original shape (except the sample size dimension)
        original_shape = data_input_np.shape[1:]
        data_input_normalized_np = data_input_normalized_flattened.reshape(-1, *original_shape)
