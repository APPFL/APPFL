...
def load_funcx_config(cfg: FuncXConfig, config_file: str):
...
def load_funcx_device_config(cfg: FuncXConfig, config_file: str):
    assert osp.exists(config_file), "Config file {config_file} not found!"
    with open(config_file) as fi:
        data = yaml.load(fi, Loader=yaml.SafeLoader)
