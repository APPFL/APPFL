...
class ServerAgent:
    ...
    def data_readiness_report(self, readiness_report: Dict) -> None:
        output_dir = self.server_agent_config.data_readiness_configs.get("output_dirname", "./output")
        output_filename = self.server_agent_config.data_readiness_configs.get("output_filename", "data_readiness_report")

        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # Save JSON report
        json_file_path = self._get_unique_file_path(output_dir, output_filename, "json")
        self._save_json_report(json_file_path, readiness_report)

        # Generate and save HTML report
        html_file_path = self._get_unique_file_path(output_dir, output_filename, "html")
        html_content = self._generate_html_content(readiness_report)
        self._save_html_report(html_file_path, html_content)

        self._data_readiness_reports = {}

    def _get_unique_file_path(self, output_dir, output_filename, extension):
    ...
    def _save_json_report(self, file_path, readiness_report):
        with open(file_path, 'w') as json_file:
            json.dump(readiness_report, json_file, indent=4)
        self.logger.info(f"Data readiness report saved as JSON to: {file_path}")

    def _save_html_report(self, file_path, html_content):
        with open(file_path, 'w') as html_file:
            html_file.write(html_content)
        self.logger.info(f"Data readiness report saved as HTML: {file_path}")

    def _generate_html_content(self, readiness_report):
        ...
        for client_id in client_ids:
            ...
            html_content += self._add_client_section(client_id, attribute_keys, plots, readiness_report)
    ...
    def _add_client_section(self, client_id, attributes, plots, readiness_report):
