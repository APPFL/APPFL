...
def construct_tensor_record(name, nparray):    
    return TensorRecord(name=name, data_shape=list(nparray.shape), data_bytes=nparray.tobytes(order='C'), data_dtype=str(nparray.dtype))
...
def proto_to_databuffer(proto, max_message_size=(2*1024*1024)):
    ...
    message_size = data_bytes_size if max_message_size > data_bytes_size else max_message_size
