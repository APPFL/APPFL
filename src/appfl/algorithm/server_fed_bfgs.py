...
class ServerFedBFGS(FedServer):
    def compute_step(self):                
        ...
        name = "conv1.bias"
        print("BEFORE=", self.pseudo_grad[name][1])
        print("self.model_size[name]=", self.model_size[name])
        ## to a vector
        ...
        print(type(self.pseudo_grad_vec[name]))
        
        ## recover from a vector
        self.pseudo_grad[name] = torch.reshape( self.pseudo_grad_vec[name], self.model_size[name] )

        print("AFTER=",self.pseudo_grad[name][1])
