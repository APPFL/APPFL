...
class FedresServer(BaseServer):
    ...
    def update(self, local_states: OrderedDict):        
        ...
        for name, param in self.model.named_parameters():
            ...
            self.global_state[name] = 0.5*( self.primal_states[xrt_idx][name] - y[name])
...
class FedresClient(BaseClient):
    ...
    def update(self):
        ...
        for name, param in self.model.named_parameters(): ## model parameters received from the server
            self.global_state[name] = param.data  
        
        if self.id < self.cfg.num_clients-1: ## initial points for XRF clients                    
            for name, param in self.model.named_parameters():
                param.data = self.local_state_prev[name] + self.cfg.fed.args.coeff[self.id]*self.global_state[name]
