...
class FedYogiAggregator(FedAvgAggregator):
    ...
    def __init__(
        ...
        log_optimization_status("FedYogiAggregator", self.optimize_memory, self.logger)
        ...
        if self.optimize_memory:
            with torch.no_grad():
                for name in self.step:
                    ...
                    yogi_scaled = yogi_update * (1 - self.aggregator_configs.server_momentum_param_2)
                    self.v_vector[name] = safe_inplace_operation(self.v_vector[name], 'sub', yogi_scaled)
                    ...
                    optimize_memory_cleanup(momentum_term, step_term, step_squared, sign_term, yogi_update, yogi_scaled, numerator, denominator, force_gc=False)
