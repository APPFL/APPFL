...
class FedAdagradAggregator(FedAvgAggregator):
    ...
    def __init__(
        ...
        log_optimization_status("FedAdagradAggregator", self.optimize_memory, self.logger)
        ...
        if self.optimize_memory:
            with torch.no_grad():
                for name in self.step:
                    ...
                    self.v_vector[name] = safe_inplace_operation(self.v_vector[name], 'add', step_squared)
                    ...
                    optimize_memory_cleanup(momentum_term, step_term, step_squared, numerator, denominator, force_gc=False)
