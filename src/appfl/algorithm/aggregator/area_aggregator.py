...
class AREAAggregator(FedAsyncAggregator):
    """
    AREA Aggregator class for Federated Learning.
    For more details, check paper: https://arxiv.org/pdf/2405.10123
    """
    def __init__(
        self,
    ...
    def aggregate(self, client_id: Union[str, int], local_model: Union[Dict, OrderedDict], **kwargs) -> Dict:
        ...
        if self.counter == self.K:
            ...
            self.counter = 0
            for name in self.global_state:
                self.pseudo_grad[name] = torch.zeros_like(self.global_state[name])
