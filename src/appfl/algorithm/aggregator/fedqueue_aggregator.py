...
from appfl.misc.memory_utils import optimize_memory_cleanup, safe_inplace_operation, clone_state_dict_optimized
...
class FedQueueAggregator(BaseAggregator):
    ...
    def aggregate(
        ...
        aggregation_factors = {
            client_id: self.staleness_fn(staleness[client_id]) * (local_steps[client_id] / local_step_sum)
                ...
                for i, client_id in enumerate(local_models):
                    ...
                    for name in self.global_state:
                        if (
                            ...
                            if gradient_based:
                                self.global_state[name] = safe_inplace_operation(
                                    self.global_state[name], "sub", local_model[name] * self.aggregation_factors[client_id]
                                )
                            else:
                                global_state_cp[name] = safe_inplace_operation(
                                    global_state_cp[name], "add", local_model[name] * self.aggregation_factors[client_id]
                                )
                                if i == num_clients - 1:
