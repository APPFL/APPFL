...
class FedServerPCA3(BaseServer):
    def __init__(self, weights, model, num_clients, device, **kwargs):
        super(FedServerPCA3, self).__init__(weights, model, num_clients, device)
        ...
        for id in range(self.num_clients):
            self.P[id], self.EVR[id] = super(FedServerPCA3, self).construct_projection_matrix(id)            
    ...
    def update(self, local_states: OrderedDict):
        ...
        global_state_vec = super(FedServerPCA3, self).get_model_param_vec()        
        global_state_vec = torch.tensor(global_state_vec, device = self.device)
        global_state_vec = global_state_vec.reshape(-1,1)
 
        super(FedServerPCA3, self).reduced_grad_vec_recover_from_local_states(local_states)
        ...
        super(FedServerPCA3, self).update_param(global_state_vec)
 

    def logging_iteration(self, cfg, logger, t):
        if t == 0:
            title = super(FedServerPCA3, self).log_title()
            logger.info(title)

        contents = super(FedServerPCA3, self).log_contents(cfg, t)
        logger.info(contents)

    def logging_summary(self, cfg, logger):
        super(FedServerPCA3, self).log_summary(cfg, logger)
