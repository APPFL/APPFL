...
class ServerFedSDLBFGSLayer(FedServer):
    ...
    def __init__(self, *args, **kwargs):
        super(ServerFedSDLBFGSLayer, self).__init__(*args, **kwargs)
    ...
    def _add_grad(self, updates):
        for j, param in zip(updates, self.model.named_parameters()):
            step_size, update = j
            name, _ = param
            p = self.model.state_dict()[name]
            self.step[name] = step_size * update.view_as(p.data)
