...
class CompassScheduler(BaseScheduler):
    ...
    def _record_info(self, client_id: Union[int, str]) -> None:
        ...
        client_start_time = (
            ...
            if client_id in self.client_info else 0
        )
        ...
        if client_id not in self.client_info:
            ...
            self.client_info[client_id]['speed'] = (
                (1 - self.scheduler_configs.speed_momentum) * self.client_info[client_id]['speed'] + 
    ...
    def _single_update(
        ...
        if not buffer:
            global_model = self.aggregator.aggregate(
                client_id,
                local_model,
                staleness=self.global_timestamp-self.client_info[client_id]['timestamp'],
    ...
    def _join_group(
        ...
        for group in self.arrival_group:
            ...
            local_steps = (
                math.floor(remaining_time / self.client_info[client_id]['speed'])
            )
            if (local_steps < self.scheduler_configs.min_local_steps or
                local_steps < assigned_steps or
                local_steps > self.scheduler_configs.max_local_steps
            ):
                continue
            else:
    ...
    def _create_group(
        ...
        for group in self.arrival_group:
            if curr_time < self.arrival_group[group]['latest_arrival_time']:
                ...
                local_steps = (
                    math.floor((est_arrival_time - curr_time) / 
                    self.client_info[client_id]['speed'])
                )
        ...
        assigned_steps = (
            ...
            if assigned_steps >= 0 and assigned_steps < self.scheduler_configs.min_local_steps
            else assigned_steps
        )
        assigned_steps = (
            self.scheduler_configs.max_local_steps
