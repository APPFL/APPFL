def get_data(cfg, client_idx: int):
    ...
    class FromCSVDataset(Dataset):
        def __init__(self, path, start_line=-1, end_line=-1):
            ...
            with open(path) as paths_file:
                ...
                for line in csv_:
                    if num_iter_left <= 0 and start_line != -1:
                        break
                    num_iter_left -= 1
                    xray_path = line[0]
                    label = line[5:]
                    ...
                    for idx, label_ in enumerate(label):
                        if label_:
                            label[idx] = float(label_)
                        if not label_ or float(label_) == -1:
                            label[idx] = 0
        ...
        def __getitem__(self, idx):
            ...
            )  # DenseNet takes 224*224 images
            transforms_list.append(transforms.ToTensor())
            transform = transforms.Compose(transforms_list)

            xray_path = self.xrays[idx]
            xray = transform(
                Image.open(xray_path).convert("RGB")
            )  # Although grayscale, DenseNet takes 3 channel
    ...
    train_dataset = FromCSVDataset(
        data_dir, split_size * client_idx, split_size * (client_idx + 1)
    )
    ...
    class DenseNet121(nn.Module):
        ...
        def __init__(self, num_output=14):
            ...
            self.densenet121.classifier = nn.Sequential(
                nn.Linear(num_features, num_output), nn.Sigmoid()
            )
