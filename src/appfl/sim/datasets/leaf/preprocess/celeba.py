...
def preprocess(root):
    ...
    def _get_metadata(path):
        with open(os.path.join(path, "raw", "identity_CelebA.txt")) as f_identities:
            identities = f_identities.read().split("\n")
        with open(os.path.join(path, "raw", "list_attr_celeba.txt")) as f_attributes:
    ...
    def _get_celebrities_and_images(identities, min_samples=5):
        ...
        good_celebs = {
            c: all_celebs[c] for c in all_celebs if len(all_celebs[c]) >= min_samples
        }
        return good_celebs

    def _get_celebrities_and_target(
    ...
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Get meta-data...!")
    identities, attributes = _get_metadata(path)
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] ...finished parsing meta-data (IDs and attributes)!"
    )

    # filter out celebs
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Construct celeb-image hashmap...!")
    celebrities = _get_celebrities_and_images(identities)
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] ...constructed celeb-image hashmap!")

    # filter out targets
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Construct inputs-targets hashmap...!")
    targets = _get_celebrities_and_target(celebrities, attributes)
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] ...constructed inputs-targets hashmap!")
