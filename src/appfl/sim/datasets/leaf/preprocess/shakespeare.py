...
def preprocess(root):
    ...
    def _split_into_plays(shakespeare_full):
        ...
        for i, line in enumerate(slines):
            if i > 124195 - start_i:  # this marks the end of the plays in the file.
                break
            if (
                "by William Shakespeare" in line
        ...
        return [
            play for play in plays if len(play[1]) > 1
        ], discarded_lines  # # remove degenerate "plays"
    ...
    def _convert_to_json(path):
        ...
        users_and_plays_path = os.path.join(
            path, "intermediate", "users_and_plays.json"
        )

        with open(users_and_plays_path) as file:
    ...
    # load raw data
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Read raw data...!")
    with open(os.path.join(path, "raw", RAW_FILENAME)) as input_file:
        shakespeare_full = input_file.read()
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] ...loaded raw data!")

    # split raw data into plays
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Split raw data into plays...!")
    plays, discarded_lines = _split_into_plays(shakespeare_full)
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] ...done splitting raw data into plays (discarded lines: {len(discarded_lines)})!"
    )
    ...
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Collect all examples...!")
    users_and_plays, all_examples = _get_all_examples(plays)

    ## save users-plays pair
    with open(
        os.path.join(path, "intermediate", "users_and_plays.json"), "w"
    ) as outfile:
        json.dump(users_and_plays, outfile)
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] ...collected all examples!")

    # save all examples
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] Write as files...!")
    _write_data(all_examples, os.path.join(path, "by_play_and_character"))
    logger.info(f"[LEAF-{DATASET_NAME.upper()}] ...wrote files!")

    # prase and convert to json format
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] Convert data to json format... (this may take several minutes)!"
    )
    _convert_to_json(path)
    logger.info(
        f"[LEAF-{DATASET_NAME.upper()}] ...finished converting data to json format!"
    )
