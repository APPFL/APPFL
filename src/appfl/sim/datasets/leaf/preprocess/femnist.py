...
def preprocess(root):
    ...
    _parse_files(path)
    logger.info(f'[LEAF-{DATASET_NAME.upper()}] ...finished extracting files of raw images!')
    ...
    # match by hashes
    logger.info(f'[LEAF-{DATASET_NAME.upper()}] Assign class labels to write images...!')
    _match_by_hashes(path)
    logger.info(f'[LEAF-{DATASET_NAME.upper()}] ...finished assigning class labels to write images!')
