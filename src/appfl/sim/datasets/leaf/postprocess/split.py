    ...
    for f in files:
        ...
        for i, u in enumerate(data["users"]):
            ...
            if curr_num_samples >= 2:
                ...
                num_train_samples = max(1, int((1.0 - test_size) * curr_num_samples))
                if curr_num_samples == 2:
                    num_train_samples = 1
        ...
        with open(
            os.path.join(
                data_dir, "train", f"{f[:-5]}_train_0{str(1.0 - test_size)[2:]}.json"
            ),
            "w",
        ) as outfile:
        ...
        with open(
            os.path.join(data_dir, "test", f"{f[:-5]}_test_0{str(test_size)[2:]}.json"),
            "w",
        ) as outfile:
