...
def postprocess_leaf(
    ...
    active_logger.info(
        f"[LEAF-{dataset_name.upper()}] checking preprocessing artifacts."
    )
    ...
    active_logger.info(
        f"[LEAF-{dataset_name.upper()}] preprocessing artifacts are ready."
    )
    ...
    active_logger.info(
        f"[LEAF-{dataset_name.upper()}] filtering clients by minimum sample threshold."
    )
    if not os.path.exists(f"{root}/{dataset_name}/rem_clients_data") and (
    ...
    active_logger.info(
        f"[LEAF-{dataset_name.upper()}] splitting clients into train/test sets."
    )
    if (not os.path.exists(f"{root}/{dataset_name}/train")) or (
        not os.path.exists(f"{root}/{dataset_name}/test")
    ):
    ...
    train_data = [
        file
        for file in os.listdir(os.path.join(root, dataset_name, "train"))
        if file.endswith(".json")
    ][0]
    num_clients = len(
        json.load(open(f"{root}/{dataset_name}/train/{train_data}"))["users"]
    )
    return num_clients
