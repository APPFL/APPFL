...
class M5(torch.nn.Module):
    def __init__(self, in_channels, hidden_size, num_classes):
        ...
        self.features = torch.nn.Sequential(
            torch.nn.Conv1d(
                self.in_channels, self.num_hiddens, kernel_size=80, stride=4
            ),
            torch.nn.BatchNorm1d(self.num_hiddens),
            torch.nn.ReLU(True),
            torch.nn.MaxPool1d(4),
            torch.nn.Conv1d(
                self.num_hiddens, self.num_hiddens, kernel_size=3, stride=1, padding=1
            ),
            torch.nn.BatchNorm1d(self.num_hiddens),
            torch.nn.ReLU(True),
            torch.nn.MaxPool1d(4),
        ...
        self.classifier = torch.nn.Sequential(
            Lambda(
                lambda x: torch.nn.functional.avg_pool1d(x, x.shape[-1]).permute(
                    0, 2, 1
                )
            ),
