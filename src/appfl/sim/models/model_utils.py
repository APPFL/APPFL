...
class InvertedResidualBlock(torch.nn.Module):
    def __init__(
        self, inputs, hidden_dims, outputs, kernel_size, stride, use_se, use_hardswish
    ):
        super().__init__()
        ...
        if inputs == hidden_dims:
            self.conv = torch.nn.Sequential(
                ...
                SELayer(hidden_dims)
                if use_se
                else torch.nn.Identity(),  # squeeze-excite block
                torch.nn.Conv2d(
                    hidden_dims, outputs, 1, 1, 0, bias=False
            ...
            self.conv = torch.nn.Sequential(
                torch.nn.Conv2d(
                    inputs, hidden_dims, 1, 1, 0, bias=False
                ),  # point-wise convolution
                torch.nn.BatchNorm2d(hidden_dims),
                torch.nn.Hardswish(True) if use_hardswish else torch.nn.ReLU(True),
                torch.nn.Conv2d(
...
class SandGlassLayer(torch.nn.Module):
    def __init__(self, inputs, outputs, stride, reduction_ratio):
        ...
        self.conv = torch.nn.Sequential(
            torch.nn.Conv2d(
                inputs, inputs, 3, 1, 1, groups=inputs, bias=False
            ),  # depth-wise convolution
            torch.nn.BatchNorm2d(inputs),
            torch.nn.ReLU6(True),
            torch.nn.Conv2d(
                inputs, hidden_dim, 1, 1, 0, bias=False
            ),  # point-wise convolution
            torch.nn.BatchNorm2d(hidden_dim),
            torch.nn.Conv2d(
                hidden_dim, outputs, 1, 1, 0, bias=False
            ),  # point-wise convolution
            torch.nn.BatchNorm2d(outputs),
            torch.nn.ReLU6(True),
            torch.nn.Conv2d(
                outputs, outputs, 3, stride, 1, groups=outputs, bias=False
            ),  # depth-wise convolution
            torch.nn.BatchNorm2d(outputs),
        )
...
class SNXBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, stride, reduction=0.5):
        ...
        self.squeeze = torch.nn.Sequential(
            torch.nn.Conv2d(
                in_channels, int(in_channels * reduction), 1, stride, bias=False
            ),
            torch.nn.BatchNorm2d(int(in_channels * reduction)),
            torch.nn.ReLU(True),
            torch.nn.Conv2d(
                int(in_channels * reduction),
                int(in_channels * reduction * 0.5),
                1,
                1,
                bias=False,
            ),
            ...
            torch.nn.ReLU(True),
            torch.nn.Conv2d(
                int(in_channels * reduction), out_channels, 1, 1, bias=False
            ),
...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(
            lambda t: einops.rearrange(t, "b p n (h d) -> b p h n d", h=self.heads), qkv
        )
    ...
    def __init__(self, inputs, outputs, stride=1, expansion=4):
        ...
        if expansion == 1:
            self.conv = torch.nn.Sequential(
                ...
                torch.nn.SiLU(True),
                # pw-linear
                torch.nn.Conv2d(hidden_dim, outputs, 1, 1, 0, bias=False),
                torch.nn.BatchNorm2d(outputs),
            )
        else:
            self.conv = torch.nn.Sequential(
...
class UNet3DDecoder(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        upsampling_type = (
            "trilinear" if upsampling_type == "linear" else upsampling_type
        )
...
class DANetBlock(torch.nn.Module):
    def __init__(
        self, input_dim, base_outdim, k, virtual_batch_size, fix_input_dim, drop_rate
    ):
        super().__init__()
