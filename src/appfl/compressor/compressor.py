...
class Compressor:
    ...
    def compress(self, ori_data : np.ndarray) -> Tuple[np.ndarray, float]:
        """
        ...
        if self.cfg.compressor == "SZ3" or "SZ2":
            ...
            return self.compressor_class.compress(
                data=ori_data,
                eb_mode=error_mode,
                eb_abs=error_bound,
                eb_rel=error_bound,
    ...
    def decompress(self, cmp_data : np.ndarray, ori_shape : Tuple[int, ...], ori_dtype : np.dtype) -> np.ndarray:
        """
        ...
        if self.cfg.compressor == "SZ3" or "SZ2":
            if self.compressor_class is None:
                self.compressor_class = pysz.SZ(szpath=self.cfg.compressor_lib_path)
            return self.compressor_class.decompress(
                data_cmpr=cmp_data,
