...
class NaiveSimTrainer(BaseTrainer):
    """
    NaiveSimTrainer:
        Naive simulation trainer for FL clients, which trains the model using `torch.optim` 
        optimizers for a certain number of local epochs or local steps, and make sure that
        each epoch or steps takes longer than the minimum time unit (e.g., 1 second) for 
        simulation purpose. 
    """  
    def __init__(
        self,
    ...
    def train(self):
        ...
        if not hasattr(self, '_first_start_time'):
            self._first_start_time = start_time
        
        self.start_times.append(start_time-self._first_start_time)
        ...
        # Wait for the minimum time unit
        if self.train_configs.mode == "epoch":
            if hasattr(self.train_configs, "time_per_epoch"):
                time_per_epoch = self.train_configs.time_per_epoch
                if hasattr(self.train_configs, "time_per_epoch_std"):
                    time_per_epoch_std = self.train_configs.time_per_epoch_std
                    time_per_epoch = np.random.normal(time_per_epoch, time_per_epoch_std)
                total_wait_time = time_per_epoch * self.train_configs.num_local_epochs
                self.logger.info(f"Waiting for {total_wait_time} seconds in total")
                time_elapsed = time.time() - start_time
                while time_elapsed < total_wait_time:
                    time.sleep(1)
                    time_elapsed = time.time() - start_time
        else:
            if hasattr(self.train_configs, "time_per_step"):
                time_per_step = self.train_configs.time_per_step
                if hasattr(self.train_configs, "time_per_step_std"):
                    time_per_step_std = self.train_configs.time_per_step_std
                    time_per_step = np.random.normal(time_per_step, time_per_step_std)
                total_wait_time = time_per_step * self.train_configs.num_local_steps
                self.logger.info(f"Waiting for {total_wait_time} seconds in total")
                time_elapsed = time.time() - start_time
                while time_elapsed < total_wait_time:
                    time.sleep(1)
                    time_elapsed = time.time() - start_time
        self.end_times.append(time.time()-self._first_start_time)
        self.logger.info(f"Start times: {self.start_times}")
        self.logger.info(f"End times: {self.end_times}")
